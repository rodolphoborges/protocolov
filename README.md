...
O sistema monitora automaticamente o desempenho dos agentes, recompensa o jogo em equipe (Sinergia) e gerencia vagas limitadas em esquadrões de elite, operando com uma estética **Brutalista Geométrica** brutal inspirada na interface e na Lore nativa do Valorant.
...
* 🤖 **Terminal de Inteligência (Telegram Bot):** Um bot integrado que permite a vinculação de contas (`/vincular`), transferências de esquadrão (`/unidade`), relatórios no ciberespaço (`/perfil`) e chamadas de reforço LFG globais pelo site usando o comando (`/convocar [CÓDIGO]`).
* 🚨 **Dashboard de Central de Inteligência:** Novo painel superior atualizado a cada 5 minutos via Supabase que extrai dados mortos para calcular:
  ...
  * **[ALERTA TÁTICO] Sinal Perdido:** Deteta e exibe num radar vermelho piscante os "Lobos Solitários" que jogam partidas Competitive sem a equipa.
...
* 🐺 **Estatísticas Criptografadas por Jogador:** O Player Card de cada Agente lista nativamente a sua **Trilha de Especialidade (Agente Mais Jogado)** e a **Taxa Média de Headshot (HS%)** a partir do histórico registado.
...
* 👁️ **DOSSIÊ "REDE DE INFORMAÇÕES":** O manual de operações do utilizador (Página de Briefing) sofreu *hi-jack* da personagem de inteligência cibernética **Cypher**. As suas regras de engajamento são lidas do ponto de vista deste Agente que vigia "cada um dos seus rastros de dados".
...
* **Frontend Brutalista:** HTML5 + Vanilla JS + CSS3 nativo (`clip-path`, animações de interfave `blink`, Integração Media Oficial da Riot via Valorant-API). A comunicação backend do FrontEnd ocorre assíncrona com via CDN pelo **Supabase Client**.
* **Backends Autômatos:** Scripts em Node.js assíncronos que usam **GitHub Actions (`update.yml`, `reset-dm.yml`)** em infraestruturas CI/CD com Agendamento Cron para raspagens a cada 30min invisíveis aos jogadores.
...
Para forçar a extração temporal manual dos dados de rotina dos match histories:
```bash
node update-data.js
```
...
2. **Rádio Limpo:** Silêncio operacional. Perdeu o combate? Abateu zero inimigos? Indique o dano infligido, reporte o posicionamento tático e **mute o seu som**. Desabafos bloqueiam passos no radar.
